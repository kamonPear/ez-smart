...
class CustomCalendar extends StatefulWidget {
  ...
  final List<DateTime>? markedDates;       // 🌟 1. เพิ่มตัวแปรสำหรับรับรายการวันที่ต้องการมาร์กจุด
...
class _CustomCalendarState extends State<CustomCalendar> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min, // 🌟 แก้ไข 2: เพิ่มบรรทัดนี้เพื่อให้ Column สูงพอดีกับเนื้อหาด้านใน
        children: [
          Row(
          ...
          GridView.builder(
            ...
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 7,
              // 🌟 แก้ไขตรงนี้: เปลี่ยนจาก 1.2 เป็น 1.0 หรือ 0.9 เพื่อให้ช่องวันที่สูงขึ้น (เป็นสี่เหลี่ยมจัตุรัสมากขึ้น)
            ...
            itemBuilder: (context, index) {
              ...
              bool hasMarker = widget.markedDates?.any((markedDate) =>
                  markedDate.year == currentMonth.year &&
                  markedDate.month == currentMonth.month &&
                  markedDate.day == day) ?? false;

              return GestureDetector(
                onTap: () {
                  DateTime newSelected = DateTime(currentMonth.year, currentMonth.month, day);
                  setState(() {
                ...
                child: Container(
                  ...
                  child: Column( // 🌟 4. เปลี่ยนจาก Center เป็น Column เพื่อให้ใส่จุดใต้ตัวเลขได้
                      ...
                      if (hasMarker)
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: isSelected ? Colors.black : const Color(0xFF66E07A),
                            shape: BoxShape.circle,
                          ),
                        )
                      else
                        const SizedBox(height: 7), // เผื่อพื้นที่ว่างไว้เท่ากัน เพื่อให้ตัวเลขอยู่ในระดับระนาบเดียวกันตลอด
                    ],
                  ),
                ),
              );
